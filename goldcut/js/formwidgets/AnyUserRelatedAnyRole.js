...
function rtInit(domid, rt) {
	...
	window.addEvent('domready', function () {
		...
		var vars3 = function (d) {
			$$('#' + domid + ' .m3')[0].empty();
			d.each(function (r) {
				var el2 = new Element('a', {'text':r.title, 'class':'m2var', 'data-urn':r.urn});
				//console.log(el2);
				$$('#' + domid + ' .m3')[0].grab(el2)
				el2.addEvent('click', activeRelationsOnClick)
			})
		...
		var vars = function (d) {
			$$('#' + domid + ' .m2')[0].empty();
			d.each(function (r) {
				var el = new Element('a', {'text':r.title, 'class':'m2var', 'data-urn':r.urn});
				$$('#' + domid + ' .m2')[0].grab(el);
		...
		$$('#' + domid + ' .myInput')[0].addEvent('keyup', function () {
			...
			if (old != current_value && current_value.length > 2) {
				m = {}
				m.action = 'load'
